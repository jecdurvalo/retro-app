...
export default function ManagementQuality({ plans }: { plans: ManagementPlan[] }) {
  ...
  return (
    <section aria-label="Qualidade da gestão" className="mt-4 rounded-[2rem] border border-black/5 bg-white/88 p-5 shadow-xl shadow-zinc-950/5 backdrop-blur-xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--retro-wine)]"><ClipboardCheck size={14} /> Qualidade da Gestão</p>
        ...
        <span className="rounded-xl bg-zinc-100 px-3 py-1.5 text-xs font-semibold text-zinc-500">Atualização automática</span>
      </div>

      <div className="mt-5 grid gap-4 xl:grid-cols-[0.7fr_1.3fr]">
        <article className="rounded-3xl bg-[var(--retro-wine)] p-5 text-white">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-white/55">Score geral</p>
        ...
        <article className="rounded-3xl border border-zinc-200 bg-white p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-400">Scores por dimensão</p>
          <div className="mt-4 space-y-4">
            {Object.values(quality.dimensions).map(dimension => (
              <div key={dimension.id}>
                <div className="flex items-end justify-between gap-3"><p className="text-xs font-semibold text-zinc-700">{dimension.label}</p><p className="text-sm font-semibold text-zinc-900">{dimension.score}%</p></div>
        ...
        {quality.topGaps.map(gap => (
          <article key={gap.metricId} className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <div className="flex items-start justify-between gap-3">
              <span className="rounded-lg bg-white px-2 py-1 text-[12px] font-semibold uppercase tracking-[0.1em] text-amber-700">{quality.dimensions[gap.dimension].label}</span>
              <span className="text-sm font-semibold text-amber-800">{gap.score}%</span>
            </div>
            <p className="mt-3 text-sm font-semibold text-zinc-800">{gap.title}</p>
      ...
      <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-cyan-200 bg-cyan-50 p-4 sm:flex-row sm:items-start">
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-cyan-700">Recomendação prática para o próximo ciclo</p>
