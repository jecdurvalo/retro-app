...
export default function ExecutiveReading({
  ...
  return (
    <section aria-label="Leitura executiva" className="mt-4 rounded-[2rem] border border-black/5 bg-white/88 p-5 shadow-xl shadow-zinc-950/5 backdrop-blur-xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--retro-wine)]"><Sparkles size={14} /> Leitura Executiva</p>
        ...
        <div className="flex flex-wrap gap-2">
          {(Object.keys(toneLabels) as ExecutiveReadingTone[]).map(value => (
            <button key={value} onClick={() => setTone(value)} aria-pressed={tone === value} className={`rounded-xl px-3 py-2 text-xs font-semibold ${tone === value ? 'bg-zinc-900 text-white' : 'border border-zinc-200 bg-white text-zinc-500'}`}>{toneLabels[value]}</button>
          ))}
          <button onClick={generate} className="inline-flex items-center gap-2 rounded-xl bg-[var(--retro-wine)] px-4 py-2 text-xs font-semibold text-white">{generated ? <RefreshCw size={14} /> : <BookOpenCheck size={14} />}{generated ? 'Atualizar leitura' : 'Gerar leitura'}</button>
        </div>
      </div>
      ...
      {!generated ? (
        <div className="mt-5 rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 p-6 text-center">
          ...
          <p className="mt-3 text-sm font-semibold text-zinc-700">Gere uma leitura baseada apenas nos dados registrados.</p>
          <p className="mt-1 text-xs font-semibold text-zinc-400">Quando faltarem evidências, os gaps serão explicitados.</p>
        </div>
      ) : (
        <div className="mt-5 space-y-4" aria-live="polite">
          <p className="flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.1em] text-zinc-400"><CalendarClock size={13} /> Gerada em {generatedAt} · {toneLabels[tone]}</p>
          <article className="rounded-2xl bg-[var(--retro-wine)] p-5 text-white">
            <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-white/55">1. Resumo executivo</h3>
          ...
          {reading.gaps.length > 0 && (
            <details className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
              <summary className="cursor-pointer text-xs font-semibold text-zinc-600">Limitações desta leitura ({reading.gaps.length})</summary>
...
function ReadingList({ title, items, empty }: { title: string; items: string[]; empty: string }) {
  return (
    <article className="rounded-2xl border border-zinc-200 bg-white p-4">
      <h3 className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--retro-wine)]">{title}</h3>
