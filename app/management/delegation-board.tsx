...
export default function DelegationBoard() {
  ...
  return (
    <section aria-label="Delegação e autonomia" className="mt-4 rounded-[2rem] border border-black/5 bg-white/88 p-5 shadow-xl shadow-zinc-950/5 backdrop-blur-xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--retro-wine)]"><UsersRound size={14} /> Delegação e autonomia</p>
        ...
        <div className="flex flex-wrap gap-2">
          <label className="rounded-2xl border border-zinc-200 bg-zinc-50 px-3 py-2">
            <span className="block text-[12px] font-semibold uppercase tracking-[0.1em] text-zinc-400">Gestora de referência</span>
            <input aria-label="Gestora de referência" value={manager} onChange={event => setManager(event.target.value)} className="mt-0.5 w-36 bg-transparent text-xs font-semibold text-zinc-700 outline-none focus-visible:ring-2 focus-visible:ring-[var(--retro-wine)] focus-visible:ring-offset-1" />
          </label>
          <button onClick={addItem} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[var(--retro-wine)] px-4 py-3 text-sm font-semibold text-white"><Plus size={16} /> Delegar responsabilidade</button>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_1.6fr]">
        <article className="rounded-3xl bg-[var(--retro-wine)] p-5 text-white">
          <div className="flex items-start justify-between gap-3">
            <div><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-white/55"><Gauge size={14} /> Índice de Delegação</p><p className="mt-3 text-5xl font-black">{index.overall}%</p></div>
            <span className="rounded-2xl bg-white/10 px-3 py-2 text-xs font-semibold text-white/65">{items.length} itens</span>
          </div>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {[['Iniciativas com DRI claro', index.dri], ['Sem dependência direta', index.independent], ['Check-ins em dia', index.checkIns], ['Com critério de sucesso', index.criteria]].map(([label, value]) => (
              <div key={label} className="rounded-2xl bg-white/[0.09] p-3"><p className="text-2xl font-black">{value}%</p><p className="mt-1 text-xs font-semibold text-white/60">{label}</p></div>
            ))}
          </div>
        </article>
        <div className="grid gap-3 sm:grid-cols-2">
          {alerts.map(alert => (
            <article key={alert.label} className={`rounded-2xl border p-4 ${alert.value ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}>
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-sm font-semibold text-zinc-800">{alert.label}</p><p className="mt-1 text-xs font-semibold text-zinc-500">{alert.detail}</p></div>
                <span className={`grid h-9 w-9 place-items-center rounded-xl text-sm font-semibold ${alert.value ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-700'}`}>{alert.value || <CheckCircle2 size={16} />}</span>
              </div>
            </article>
          ))}
        </div>
      ...
      <div className="mt-5 flex gap-2 border-b border-zinc-200">
        <button onClick={() => setView('people')} className={`border-b-2 px-3 py-3 text-xs font-semibold ${view === 'people' ? 'border-[var(--retro-wine)] text-[var(--retro-wine)]' : 'border-transparent text-zinc-400'}`}><UserRound className="mr-1.5 inline" size={14} /> Por pessoa</button>
        <button onClick={() => setView('initiatives')} className={`border-b-2 px-3 py-3 text-xs font-semibold ${view === 'initiatives' ? 'border-[var(--retro-wine)] text-[var(--retro-wine)]' : 'border-transparent text-zinc-400'}`}><BriefcaseBusiness className="mr-1.5 inline" size={14} /> Por iniciativa</button>
      </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {people.map(person => (
            <article key={person.name} className="rounded-2xl border border-zinc-200 bg-white p-4">
              <div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-zinc-900">{person.name}</p><p className="mt-1 text-xs font-semibold text-zinc-400">{person.responsibilities.length} responsabilidade(s)</p></div>{person.name === manager && <span className="rounded-lg bg-[rgba(135,0,47,0.08)] px-2 py-1 text-[12px] font-semibold text-[var(--retro-wine)]">Gestora</span>}</div>
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <div className="rounded-xl bg-rose-50 p-2"><p className="text-lg font-black text-rose-700">{person.critical}</p><p className="text-[12px] font-semibold text-rose-500">Críticos</p></div>
                <div className="rounded-xl bg-amber-50 p-2"><p className="text-lg font-black text-amber-700">{person.support}</p><p className="text-[12px] font-semibold text-amber-600">Pedem apoio</p></div>
                <div className="rounded-xl bg-cyan-50 p-2"><p className="text-lg font-black text-cyan-700">{person.autonomy}</p><p className="text-[12px] font-semibold text-cyan-600">Evoluindo</p></div>
              </div>
              <div className="mt-3 space-y-1.5">{person.responsibilities.slice(0, 3).map(item => <button key={item.id} onClick={() => setExpandedId(item.id)} className="block w-full truncate rounded-xl bg-zinc-50 px-3 py-2 text-left text-xs font-semibold text-zinc-600">{item.title}</button>)}</div>
            </article>
          ))}
        </div>
      ) : (
          ...
          {activeInitiatives.map(initiative => {
            ...
            return (
              <article key={initiative.id} className="rounded-2xl border border-zinc-200 bg-white p-4">
                <div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-zinc-900">{initiative.title}</p><p className="mt-1 text-xs font-semibold text-zinc-400">{initiative.area || 'Área não definida'}</p></div><span className="rounded-xl bg-zinc-100 px-2.5 py-1 text-[12px] font-semibold text-zinc-600">{related.length} parte(s) delegada(s)</span></div>
                <div className="mt-4 grid gap-2 sm:grid-cols-2">
                  <p className="rounded-xl bg-zinc-50 p-3 text-xs font-semibold text-zinc-500"><strong className="block text-[12px] font-semibold uppercase tracking-[0.1em] text-zinc-400">DRI</strong><span className="mt-1 block font-semibold text-zinc-700">{initiative.owner || 'Sem DRI'}</span></p>
                  <p className="rounded-xl bg-zinc-50 p-3 text-xs font-semibold text-zinc-500"><strong className="block text-[12px] font-semibold uppercase tracking-[0.1em] text-zinc-400">Apoia</strong><span className="mt-1 block font-semibold text-zinc-700">{support.join(', ') || 'Sem apoio definido'}</span></p>
                </div>
                {managerDecision && <p className="mt-2 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800"><AlertCircle className="mr-1.5 inline" size={13} /> Decisão depende da gestora: {initiative.decisionNeeded}</p>}
              </article>
            )
          })}
        </div>
          ...
          return (
            <article key={item.id} className={`overflow-hidden rounded-2xl border bg-white ${isCheckInLate(item.nextCheckIn) ? 'border-amber-200' : 'border-zinc-200'}`}>
              <button onClick={() => setExpandedId(expanded ? null : item.id)} aria-expanded={expanded} className="grid w-full gap-3 p-4 text-left lg:grid-cols-[1fr_180px_150px_36px] lg:items-center">
                <div><p className="text-sm font-semibold text-zinc-900">{item.title}</p><p className="mt-1 text-xs font-semibold text-zinc-400">{initiative?.title || 'Sem iniciativa relacionada'}</p></div>
                <div><p className="text-xs font-semibold text-zinc-700">{item.responsible || 'Sem DRI'}</p><p className="mt-1 text-[12px] font-semibold text-zinc-400">{responsibilityLabels[item.responsibilityType]}</p></div>
                <div><p className="text-xs font-semibold text-zinc-700">Autonomia {item.autonomyLevel}</p><p className={`mt-1 text-[12px] font-semibold ${isCheckInLate(item.nextCheckIn) ? 'text-amber-700' : 'text-zinc-400'}`}>{isCheckInLate(item.nextCheckIn) ? 'Check-in pendente' : `Próximo: ${new Date(`${item.nextCheckIn}T12:00:00`).toLocaleDateString('pt-BR')}`}</p></div>
              ...
              {expanded && (
                <div className="border-t border-zinc-100 bg-zinc-50/70 p-4">
                  ...
                  <div className="mt-3 flex justify-end"><button onClick={() => setItems(current => current.filter(entry => entry.id !== item.id))} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold text-rose-600"><Trash2 size={13} /> Excluir</button></div>
                </div>
              )}
            </article>
          )
